...
persist_playwriter_env() {
    ...
    {
        echo "# Generated by zeroclaw entrypoint"
        if [ -n "$host" ]; then
            printf 'export PLAYWRITER_HOST="%s"\n' "$host"
        fi
        if [ -n "$token" ]; then
            printf 'export PLAYWRITER_TOKEN="%s"\n' "$token"
        fi
    } > "$profile_file"
...
persist_shell_path() {
    local profile_file="/etc/profile.d/zeroclaw-path.sh"
    local extra_path="/usr/local/bin/agent-tools:/usr/local/bin:/opt/bun/bin"
